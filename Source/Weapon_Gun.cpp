...
Weapon_Gun2::Weapon_Gun2(Actor* actor) : Weapon(actor)
{
    ASps    = 3.5f;
    mLength = 300;
    mMode   = AimMode::MOUSE_POS;
...
void Weapon_Gun2::update(float delta)
{
    dealay += delta;
    float a = 1.f / ASps;
    if (dealay >= a)
...
void Weapon_Gun2::Attack()
{
    ...
    for (int i = 0; i < mTimes; i++)
    {
        ...
        currenttimes += x * 2 * a;
        target      = mAiming.rotateByAngle(mActor->mPosition, currenttimes);
        auto bullet = Spawn_Bullet2(mActor->mSceneComp->mRootNode->getParent(), mActor->getPosition(), mActor, target);
    }
}
